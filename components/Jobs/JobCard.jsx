...
const JobPostCard = ({ job }) => {
    return (
      ...
      <div className="w-full mb-5 relative group cursor-pointer hover:shadow-lg transition transition-all duration-300 ease-in-out h-auto px-6 py-6 bg-white rounded-[10px] flex flex-col md:flex-row gap-5 border border-gray-200">
        ...
        {/* image */}
        {job.companyLogo?
        <div className="relative w-[56px] mr-3 h-[56px] md:w-[66px] md:h-[66px] bg-[#CEE2FF] rounded-full border border-gray-100">
        ...
        <div className="flex flex-col grid gap-3 grid flex-grow">
          <div className="flex flex-col grid gap-6">
            <div className="flex flex-col grid gap-2">
              <h1 className="text-xl m-0 inter-font font-medium max-w-[400px]">{job.title}</h1>
              <div className="flex flex-row text-base inter-font font-regular text-[#A3ACC2]">
                <span className="mr-3 text-gray-800 font-medium text-md">{job.companyName}</span>
                {job.type?
                 <>
                 <div className="mr-3 flex flex-row gap-0.5 ml-1">
                  <ClockIcon/>
            ...
            <div className="flex flex-row">
              {job.skills?.map((x, i) => (
                <span
                  className="py-1 mr-2 px-3 text-xs rounded-full bg-purple-100 text-purple-600 text-md"
                  key={i}
                >
                  {x.name}
                </span>
