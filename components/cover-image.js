...
export default function CoverImage({ title, url, slug, type, route, imageHeight }) {
  ...
  const image = url ? (
    <figure className={`relative w-full ${imageHeight?imageHeight:'h-60 md:h-40'} overflow-hidden rounded-lg transform group-hover:translate-x-0 group-hover:shadow group-hover:translate-y-0 transition duration-700 ease-out overflow-hidden`}>
