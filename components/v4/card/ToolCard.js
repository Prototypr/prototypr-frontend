      ...
      {posts.map((post, i) => {
        ...
        return (
          <Link href={`/toolbox/${slug}`}>
            <div
              key={slug}
              className="w-auto group flex md:flex-row justify-between gap-6 p-5 bg-white rounded-2xl  border-opacity-10"
            >
              <div className="flex justify-center sm:flex-row gap-4">
                {coverImage ? (
                  <div
                    className=" relative rounded-2xl overflow-hidden "
                    style={{ height: "75px", width: "75px", flex: `0 0 75px` }}
                  >
                    <Image
                      loader={gumletLoader}
