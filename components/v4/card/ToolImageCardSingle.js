...
const ToolImageCardSingle = ({ post, imageLarge,type, columns, tagNumber }) => {
  ...
  return (
    <div className="h-full">
            ...
            <Link target={post.postType=='ad'?'_blank':"_self"} href={post.postType=='ad'?post.link+'?ref=prototypr':`/toolbox/${post?.slug}`}>
            <div
        className={
      ...
      >
        ...
        {/* <Image
          ...
          <div className="pl-2 overflow-hidden my-auto">
            <div className={'line-clamp-1 tracking-tight font-medium'}> {post?.title}</div>
            {/* {this.props.prototool !== true && */}
            {post?.tags?.length? (
                   <div className='text-xs text-gray-500 mt-0.5 capitalize'>{post?.tags[0].attributes.name}</div>
                ):post?.postType=='ad'?(
                  <div className='text-xs text-gray-500 mt-0.5 capitalize bg-gray-100 text-gray-800/90 rounded-xl w-[fit-content] px-1.5 py-0.5'>Ad</div>
                ):''}
