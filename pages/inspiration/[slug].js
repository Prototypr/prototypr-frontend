...
export default function Post({ post, morePosts, preview,relatedPosts }) {
                ...
                <div className="relative bg-white p-6 pt-6 rounded-lg shadow w-full gallery-rounded">
                  <div className="flex mb-6">
                    <div className="hover:bg-gray-100">
                      <a
                        href={
                          "https://blog.prototypr.io/being-a-design-intern-at-dunzo-ccebe8a6eac7?source=rss----eb297ea1161a---4?ref=prototyprio"
                        }
                      >
                        <h1 className="text-lg tracking-tight mr-2 leading-tight font-medium text-gray-900">
                          <div
                            dangerouslySetInnerHTML={{
                              __html: post.attributes.title,
                            }}
                          />
                        </h1>
                        <div className="flex mt-1">
                          ...
                          <div
                            className="md:block my-auto text-gray-500 text-xs "
                            style={{
                              paddingLeft: "0.1rem",
                              paddingRight: "0.1rem",
                            }}
                          ...
                          <TimeAgo
                            className="text-gray-700 text-base md:px-1"
                            date={d}
                          />
                        </div>
                      </a>
                  ...
                  >
                    <img
                      ...
                      src={post?.attributes.ogImage.opengraphImage}
                    />
                  </a>
                </div>
              </div>
...
export async function getStaticProps({
  ...
  let tags = data?.posts.data.length ? data?.posts.data[0].attributes.tags : [];
  let tagsArr = [];
  if (tags.data) {
...
export async function getStaticPaths() {
  const allPosts = await getAllPostsWithSlug("designspo");
  // console.log('allPosts.data******' + JSON.stringify(allPosts.data))
  return {
    paths:
      (allPosts &&
        allPosts.data?.map((post) => {
          return `/inspiration/${post.attributes.slug}`;
        })) ||
      [],
    fallback: true,
  };
