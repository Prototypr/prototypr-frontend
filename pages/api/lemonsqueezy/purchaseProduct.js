...
async function purchaseProduct(req, res) {
  ...
  try {
    ...
    let sponsorshipQuantity =
      reqData.weeks?.newsletter?.length + reqData.weeks?.website?.length;
    ...
    if (
      reqData.selectedProducts.find(product => product.type === "newsletter") &&
      reqData.selectedProducts.find(product => product.type === "website")
    ) {
      if (!addDiscount) {
        addDiscount = process.env.LEMON_SQUEEZY_10_DISCOUNT;
      } else {
    ...
    const attributes = {
      ...
      product_options: {
        enabled_variants: [
          process.env.NEXT_PUBLIC_LEMONSQUEEZY_PRODUCTID_SPONSORSHIP,
        ],
      },
      checkout_options: {
        discount: false,
...
export default async function mainHandler(req, res) {
  ...
  req.session = session;
  return purchaseProduct(req, res);
}
...
const calculatePrice = async reqData => {
  ...
  products.data.forEach(product => {
    ...
    const quantity = reqData.selectedProducts?.find(product => product.id == id)
      .dates.length;
  ...
  if (reqData.customDiscount) {
    try {
      const response = await lemonSqueezyApiInstance.get(
        `/discounts?filter[store_id]=${process.env.LEMON_SQUEEZY_STORE_ID}`
      );
      //check if discount code exists
      const discountItem = response.data.data.find(
        d => d.attributes.code === reqData.customDiscount
      );
