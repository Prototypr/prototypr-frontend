...
import TopicsSelect from "@/components/user/TopicsSelect";
...
const AccountPage = ({ preview }) => {
  ...
  useEffect(()=>{
    const getTagOptions = async() =>{
        const tags = (await getPopularTopics({postType:'article', pageSize:30, offset:0})) || [];
        setAllTags(tags)
    }
    getTagOptions()
  },[])
            ...
            <div className="pt-6 pb-10 px-0 xl:px-0">
              <div className="bg-white shadow-sm rounded-xl py-6 px-4">
                ...
                <span className="text-sm text-gray-500">
                  Topics will be displayed on your public profile
                </span>
                {allTags?<TopicsSelect
                allTags={allTags}
          ...
          <div className="pt-6 pb-10 md:pt-10 px-3 xl:px-0">
            <div className="bg-white shadow-sm rounded-xl py-6 px-4">
              <h1 className="font-semibold text-lg">Confirm your email</h1>
                ...
                Hi {user.name}, 
                please check your email ({user.email}) to confirm your account. If you didn't receive it, try again with the following form:
                </p>
                <Form 
