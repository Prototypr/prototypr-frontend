      ...
      for (let j = 0; j < sponsors[i]?.weeks?.website?.length; j++) {
        if (sponsors[i].weeks.website[j].productId === navSponsorId) {
          if(sponsors[i].active){
            navSponsor = sponsors[i];
            sponsorIsFallback=false
            break;
          }
        }
      }
      if(!navSponsor && sponsors[i].fallback){
        navSponsor = sponsors[i];
      }
    }
    if (navSponsor && !sponsorIsFallback) break; // If navSponsor is found, break the outer loop
  }
