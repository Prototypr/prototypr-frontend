export const updateSessionUser = (newStrapiProfile, sessionUser) =>{
  ...
  console.log(newStrapiProfile.email)
  console.log(sessionUser.login.user.email)
    //when the user email changes, set their confirmation status to unconfirmed
    if(newStrapiProfile.email && (newStrapiProfile.email != sessionUser.login.user.email)){
