  ...
  const id = await axios(configUpload)
    .then(async function (response) {
      if (response?.data?.posted == true) {
         ...
         * upload logoWide
         */
        if (values?.logoWide && uploadLogoWide === true) {
          const loadingLogoWide = toast.loading("Uploading your logo (wide version)...", {
            duration: 3000,
          });
          const file = new File([values.banner], `logowide_.png`, {
            type: "image/png",
          });
          ...
          await axios(imageConfig)
            .then(async function (response) {
              ...
              //set field value to id of image, which is used to attach to post
              toast.success("Logo (wide version) upload complete!", {
                duration: 3000,
              });
            })
            .catch(function (error) {
              ...
              toast.console.warn("Logo (wide version) failed to save.", {
                duration: 3000,
              });
            });
          if (loadingLogoWide) {
